...
export class PodioService {
    ...
    GetUserOrgs() {
        const p = pipe(
            map((token: string) =>
                this.http.get<Org[]>(this.BASE_URL + "org/", {
                    // options
                    headers: {
    ...
    GetOrg(orgId: Number) {
        const p = pipe(
            map((token: string) =>
                this.http.get<Org>(this.BASE_URL + "org/" + orgId, {
                    // options
                    headers: {
                        ["Authorization"]: "OAuth2 " + token
                    }
    ...
    GetWorkspace(spaceId: Number) {
        const p = pipe(
            map((token: string) =>
                this.http.get<Space>(this.BASE_URL + "space/" + spaceId, {
                    // options
                    headers: {
                        ["Authorization"]: "OAuth2 " + token
                    }
        ...
        const p = pipe(
            map((token: string) =>
                this.http.get<Application[]>(
                    this.BASE_URL + "app/space/" + spaceId,
                    {
                        // options
                        headers: {
                            ["Authorization"]: "OAuth2 " + token
    ...
    GetApp(appId: Number) {
        const p = pipe(
            map((token: string) =>
                this.http.get<Application>(this.BASE_URL + "app/" + appId, {
                    // options
                    headers: {
                        ["Authorization"]: "OAuth2 " + token
                    }
    ...
    GetFieldInApp(appId: Number, fieldId: Number) {
        const p = pipe(
            map((token: string) =>
                this.http.get<Field>(
                    this.BASE_URL + "app/" + appId + "/field/" + fieldId,
                    {
                        // options
                        headers: {
                            ["Authorization"]: "OAuth2 " + token
