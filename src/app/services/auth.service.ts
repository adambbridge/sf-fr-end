...
export class AuthService {
  ...
  constructor(private _router: Router, private http: HttpClient) {
    ...
    this.IsAuth$.subscribe({next: (b: boolean) => {
      this.IsAuth = b;
    }});
  ...
  login(): void {
    window.location.href = environment.podio.oauth.baseUrl + // create URL from parameters rather than hardcode
    '?response_type=token&client_id=' + environment.podio.oauth.clientId +
    '&redirect_uri=' + environment.podio.oauth.redirectUrl +
    '&state=12345';
  }

  loginSolutionServiceAccount(solutionId: string, version: string): void {
    window.location.href = environment.podio.oauth_service.baseUrl + // create URL from parameters rather than hardcode
    '?response_type=code&client_id=' + environment.podio.oauth_service.clientId +
    '&redirect_uri=' + environment.podio.oauth_service.redirectUrl +
    '&state=' + JSON.stringify( { solutionId , version } );
  }

  loginEnvironmentServiceAccount(clientId: string, environmentId: string): void {
    window.location.href = environment.podio.oauth_service.baseUrl + // create URL from parameters rather than hardcode
    '?response_type=code&client_id=' + environment.podio.oauth_service.clientId +
    '&redirect_uri=' + environment.podio.oauth_service.redirectUrl +
    '&state=' + JSON.stringify( { clientId , environmentId } );
  }
